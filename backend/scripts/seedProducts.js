...
// Category distribution mapping to reach EXACTLY 500 products
...
// Generate EXACTLY 500 product records
function generate500Products() {
  ...
  categoryCounts.forEach(({ prefix, category, count }) => {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const tmpl = list[catIndex % list.length];
      const packName = tmpl.pack[(i - 1) % tmpl.pack.length];
      const price = tmpl.price[(i - 1) % tmpl.price.length] || tmpl.price[0];
      const gst = tmpl.gst;

      // Realistic variation in product name
      const fullname = `${tmpl.name} ${packName}`;

      // Stock variation: High (50-200), Medium (20-50), Low (5-20), Out of stock (0)
      let stock = 45;
      if (i % 15 === 0) stock = 0; // Out of stock
      else if (i % 7 === 0) stock = Math.floor(Math.random() * 12) + 2; // Low stock (2-14)
      else if (i % 3 === 0) stock = Math.floor(Math.random() * 30) + 20; // Medium stock (20-50)
      else stock = Math.floor(Math.random() * 150) + 50; // High stock (50-200)
      ...
      products.push({
        ...
        gst_percent: parseFloat(gst).toFixed(2),
        stock
      });
...
async function seedDatabase() {
  ...
  try {
    ...
    console.log(`Generated EXACTLY ${products.length} retail products for seeding.`);
    ...
    for (let i = 0; i < products.length; i += chunkSize) {
      ...
      chunk.forEach(p => {
        values.push('(?, ?, ?, ?, ?, ?, ?)');
        queryParams.push(p.name, p.category, p.sku, p.barcode, p.price, p.gst_percent, p.stock);
      });

      const sql = `
        INSERT INTO products (name, category, sku, barcode, price, gst_percent, stock)
        VALUES ${values.join(', ')}
